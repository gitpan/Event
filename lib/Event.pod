...
dispatched by priority when it is known to be safe for callbacks to
execute.
...
C<after> attribute for easier initialization.  You can find the time
at the start of today with:

  use Time::Local;
  my $TodaySeconds = int timelocal(0,0,0,(localtime)[3,4,5]);

If C<interval> set then the watcher will automatically repeat.  Be
...
=head2 WHY WINDOWS CAN BE FASTER THAN UNIX
