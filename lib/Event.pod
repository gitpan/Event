...
implemented with an exception.  It works by effectively calling
C<unloop(undef)> for all pending loops.
...
The same as the C<start> method except in the case of watchers with
special special repeat behavior.  For example, repeating timers
recalcuate the alarm time using their C<interval> parameter.
...
L<NetServer::ProcessTop> and L<Time::HiRes>
