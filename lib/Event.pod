...
Return all queued watchers in order of priority.
...
exception is detected then C<$Event::DIED> is invoked like this:
...
Event also has a direct C API for high-performance events.  See
L<Event::MakeMaker>.
