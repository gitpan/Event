...
B<THIS IS EXPERIMENTAL AND IS LIKELY TO CHANGE BEFORE BEING DECLARED
STABLE AS GRANITE.>
...
Return a list of all events running inside its callback at the moment.
This can be more than one event!
...
Return all events in the idle queue.
...
invoked with a reference to the $event as its only additional
argument.
...
$Event::DebugLevel is treated as one level higher for this watcher.  A
level of 1, 2, or 3 give progressively more noise on STDERR.
...
the number of seconds spent within the callback.  Also see
...
mask.  The bit constants are available in Event::io.
...
L<Event::Advanced> (to be written) and L<NetServer::ProcessTop>

While not directly related, http://consult.ml.org/~trockij/mon might
also be of interest.  I'd be interested in hearing about similar
tools.
